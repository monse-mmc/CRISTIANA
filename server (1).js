...
http.createServer(async (req, res) => {
  ...
  try {
    ...
    // ── TRASLADOS (misma tabla registros con area=traslados) ──
    if (p === '/api/traslados') {
      if (req.method === 'GET') {
        const d = await sb('GET', 'registros', null, '?area=eq.traslados&order=creado_en.desc&limit=500');
        res.writeHead(200); res.end(JSON.stringify(d)); return;
      }
    }

    res.writeHead(404); res.end(JSON.stringify({error:'Ruta no encontrada: '+p}));
